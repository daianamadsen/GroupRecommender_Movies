package edu.isistan.christian.recommenders.groups.movies.magres.rA.utils;
...
public class MAGReSRAMoviesCSVExporter extends MoviesCSVResultsExporter{

	protected List<String> getUserHeaderData(GRecResult<MovieItem> result){
		...
		for (SURUser member : result.getGroup()){
			userHeaderData.add("USER [ID= "+member.getID()+"] RATED THE MOVIE?");
		}
	...
	protected List<String> getSummaryRowData (GRecResult<MovieItem> result){
		...
		for (SURUser member : result.getGroup()){
			...
			for (GRecRecommendation<MovieItem> rec : result.getRecommendations()){
				MAGReSRARecStats stats = (MAGReSRARecStats) result.getRecommendationStats(rec);
